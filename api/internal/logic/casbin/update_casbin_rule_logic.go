...
func (l *UpdateCasbinRuleLogic) UpdateCasbinRule(req *types.CasbinRuleInfo) (resp *types.BaseMsgResp, err error) {
	// 将API类型转换为RPC类型
	...
	// 调用RPC服务
	_, err = l.svcCtx.CoreRpc.UpdateCasbinRule(l.ctx, rpcReq)
	if err != nil {
		l.Logger.Errorf("Failed to update casbin rule via RPC: %v", err)
		return &types.BaseMsgResp{
			Code: 1,
			Msg:  "更新权限规则失败: " + err.Error(),
		}, nil
	}
