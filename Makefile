...
docker: # Build the docker image | 构建 docker 镜像
	docker build -f Dockerfile-api -t $(DOCKER_REPO)/$(SERVICE_DASH)-dp-api:${VERSION} .
	docker build -f Dockerfile-rpc -t $(DOCKER_REPO)/$(SERVICE_DASH)-dp-rpc:${VERSION} .
	@echo "Build docker successfully"

.PHONY: publish-docker
publish-docker: # Publish docker image | 发布 docker 镜像
	docker push $(DOCKER_REPO)/$(SERVICE_DASH)-dp-rpc:${VERSION}
	docker push $(DOCKER_REPO)/$(SERVICE_DASH)-dp-api:${VERSION}
