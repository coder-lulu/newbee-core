...
// The OauthSessionFunc type is an adapter to allow the use of ordinary function as a Querier.
type OauthSessionFunc func(context.Context, *ent.OauthSessionQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f OauthSessionFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.OauthSessionQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.OauthSessionQuery", q)
}

// The TraverseOauthSession type is an adapter to allow the use of ordinary function as Traverser.
type TraverseOauthSession func(context.Context, *ent.OauthSessionQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseOauthSession) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseOauthSession) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.OauthSessionQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.OauthSessionQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.OauthSessionQuery:
		return &query[*ent.OauthSessionQuery, predicate.OauthSession, oauthsession.OrderOption]{typ: ent.TypeOauthSession, tq: q}, nil
