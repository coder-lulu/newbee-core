...
func (du *DepartmentUpdate) AddPostIDs(ids ...uint64) *DepartmentUpdate {
	du.mutation.AddPostIDs(ids...)
	return du
}

// AddPosts adds the "posts" edges to the Position entity.
func (du *DepartmentUpdate) AddPosts(p ...*Position) *DepartmentUpdate {
...
// ClearPosts clears all "posts" edges to the Position entity.
func (du *DepartmentUpdate) ClearPosts() *DepartmentUpdate {
	du.mutation.ClearPosts()
	return du
}

// RemovePostIDs removes the "posts" edge to Position entities by IDs.
func (du *DepartmentUpdate) RemovePostIDs(ids ...uint64) *DepartmentUpdate {
	du.mutation.RemovePostIDs(ids...)
	return du
}

// RemovePosts removes "posts" edges to Position entities.
func (du *DepartmentUpdate) RemovePosts(p ...*Position) *DepartmentUpdate {
	ids := make([]uint64, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
...
func (du *DepartmentUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if du.mutation.PostsCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   department.PostsTable,
			Columns: []string{department.PostsColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
...
// AddPostIDs adds the "posts" edge to the Position entity by IDs.
func (duo *DepartmentUpdateOne) AddPostIDs(ids ...uint64) *DepartmentUpdateOne {
	duo.mutation.AddPostIDs(ids...)
	return duo
}

// AddPosts adds the "posts" edges to the Position entity.
func (duo *DepartmentUpdateOne) AddPosts(p ...*Position) *DepartmentUpdateOne {
	ids := make([]uint64, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
...
// ClearPosts clears all "posts" edges to the Position entity.
func (duo *DepartmentUpdateOne) ClearPosts() *DepartmentUpdateOne {
	duo.mutation.ClearPosts()
	return duo
}

// RemovePostIDs removes the "posts" edge to Position entities by IDs.
func (duo *DepartmentUpdateOne) RemovePostIDs(ids ...uint64) *DepartmentUpdateOne {
	duo.mutation.RemovePostIDs(ids...)
	return duo
}

// RemovePosts removes "posts" edges to Position entities.
func (duo *DepartmentUpdateOne) RemovePosts(p ...*Position) *DepartmentUpdateOne {
	ids := make([]uint64, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
