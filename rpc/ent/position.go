...
func (*Position) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case position.FieldID, position.FieldStatus, position.FieldSort, position.FieldDeptID:
...
func (po *Position) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", po.DeptID))
	builder.WriteByte(')')
