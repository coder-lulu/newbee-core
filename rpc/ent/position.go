...
func (*Position) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case position.FieldID, position.FieldStatus, position.FieldSort, position.FieldTenantID, position.FieldDeptID:
...
func (_m *Position) QueryDepartments() *DepartmentQuery {
	return NewPositionClient(_m.config).QueryDepartments(_m)
}
