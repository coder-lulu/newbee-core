...
func (dc *DepartmentCreate) AddPostIDs(ids ...uint64) *DepartmentCreate {
	dc.mutation.AddPostIDs(ids...)
	return dc
}

// AddPosts adds the "posts" edges to the Position entity.
func (dc *DepartmentCreate) AddPosts(p ...*Position) *DepartmentCreate {
	ids := make([]uint64, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
	return dc.AddPostIDs(ids...)
}
...
func (dc *DepartmentCreate) createSpec() (*Department, *sqlgraph.CreateSpec) {
	...
	if nodes := dc.mutation.PostsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
