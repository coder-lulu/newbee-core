...
func (_q *OauthAccountQuery) QueryProvider() *OauthProviderQuery {
...
func (_q *OauthAccountQuery) FirstID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (_q *OauthAccountQuery) FirstIDX(ctx context.Context) uint64 {
...
func (_q *OauthAccountQuery) OnlyID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (_q *OauthAccountQuery) OnlyIDX(ctx context.Context) uint64 {
...
func (_q *OauthAccountQuery) IDs(ctx context.Context) (ids []uint64, err error) {
...
func (_q *OauthAccountQuery) IDsX(ctx context.Context) []uint64 {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *OauthAccountQuery) WithProvider(opts ...func(*OauthProviderQuery)) *OauthAccountQuery {
	query := (&OauthProviderClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OauthAccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*OauthAccount, error) {
	...
	if query := _q.withProvider; query != nil {
		if err := _q.loadProvider(ctx, query, nodes, nil,
			func(n *OauthAccount, e *OauthProvider) { n.Edges.Provider = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *OauthAccountQuery) loadProvider(ctx context.Context, query *OauthProviderQuery, nodes []*OauthAccount, init func(*OauthAccount), assign func(*OauthAccount, *OauthProvider)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*OauthAccount)
	for i := range nodes {
...
func (_q *OauthAccountQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(oauthaccount.Table, oauthaccount.Columns, sqlgraph.NewFieldSpec(oauthaccount.FieldID, field.TypeUint64))
	_spec.From = _q.sql
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withProvider != nil {
			_spec.Node.AddColumnOnce(oauthaccount.FieldProviderID)
		}
	}
