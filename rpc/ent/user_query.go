...
func (_q *UserQuery) QueryOauthSessions() *OauthSessionQuery {
...
func (_q *UserQuery) WithOauthSessions(opts ...func(*OauthSessionQuery)) *UserQuery {
	query := (&OauthSessionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withOauthSessions; query != nil {
		if err := _q.loadOauthSessions(ctx, query, nodes,
			func(n *User) { n.Edges.OauthSessions = []*OauthSession{} },
			func(n *User, e *OauthSession) { n.Edges.OauthSessions = append(n.Edges.OauthSessions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadOauthSessions(ctx context.Context, query *OauthSessionQuery, nodes []*User, init func(*User), assign func(*User, *OauthSession)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(oauthsession.FieldUserID)
	}
	query.Where(predicate.OauthSession(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.OauthSessionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
