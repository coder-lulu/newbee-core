...
func (Menu) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("params").Optional().Default("").
			Comment("The path Params | 菜单路由参数"),
	}
}
...
func (Menu) Indexes() []ent.Index {
	return []ent.Index{
		index.Fields("name", "menu_type", "tenant_id").Unique().StorageKey("idx_menu_name_type"),
		index.Fields("path", "tenant_id").Unique(),
	}
}
