...
func (*Role) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case role.FieldCustomDeptIds:
			values[i] = new([]byte)
		case role.FieldID, role.FieldStatus, role.FieldSort, role.FieldDataScope:
	...
	for i := range columns {
		...
		case role.FieldDataScope:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				r.DataScope = uint8(value.Int64)
			}
		case role.FieldCustomDeptIds:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field custom_dept_ids", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &r.CustomDeptIds); err != nil {
...
func (r *Role) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", r.CustomDeptIds))
	builder.WriteByte(')')
