...
type Role struct {
	...
	DataScope uint8 `json:"data_scope,omitempty"`
	// Custom department setting for data permission | 自定义部门数据权限
	CustomDeptIds []uint64 `json:"custom_dept_ids,omitempty"`
...
func (*Role) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case role.FieldCustomDeptIds:
			values[i] = new([]byte)
		case role.FieldID, role.FieldStatus, role.FieldTenantID, role.FieldSort, role.FieldDataScope:
	...
	for i := range columns {
		...
		case role.FieldDataScope:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				_m.DataScope = uint8(value.Int64)
			}
		case role.FieldCustomDeptIds:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field custom_dept_ids", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.CustomDeptIds); err != nil {
					return fmt.Errorf("unmarshal field custom_dept_ids: %w", err)
				}
			}
		default:
...
func (_m *Role) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("custom_dept_ids=")
	builder.WriteString(fmt.Sprintf("%v", _m.CustomDeptIds))
	builder.WriteByte(')')
