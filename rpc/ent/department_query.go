...
func (dq *DepartmentQuery) QueryPosts() *PositionQuery {
...
func (dq *DepartmentQuery) WithPosts(opts ...func(*PositionQuery)) *DepartmentQuery {
	query := (&PositionClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (dq *DepartmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Department, error) {
	...
	if query := dq.withPosts; query != nil {
		if err := dq.loadPosts(ctx, query, nodes,
			func(n *Department) { n.Edges.Posts = []*Position{} },
			func(n *Department, e *Position) { n.Edges.Posts = append(n.Edges.Posts, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (dq *DepartmentQuery) loadPosts(ctx context.Context, query *PositionQuery, nodes []*Department, init func(*Department), assign func(*Department, *Position)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*Department)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(position.FieldDeptID)
	}
	query.Where(predicate.Position(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(department.PostsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
