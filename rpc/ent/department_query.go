...
func (_q *DepartmentQuery) QueryPosts() *PositionQuery {
...
func (_q *DepartmentQuery) WithPosts(opts ...func(*PositionQuery)) *DepartmentQuery {
	query := (&PositionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *DepartmentQuery) loadPosts(ctx context.Context, query *PositionQuery, nodes []*Department, init func(*Department), assign func(*Department, *Position)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*Department)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
