...
func (l *InitDatabaseLogic) InitDatabase(_ *core.Empty) (*core.BaseResp, error) {
	...
	if check != 0 {
		...
		// force reset admin data scope
		err = l.svcCtx.DB.Role.Update().Where(role.NameEQ("role.admin")).SetDataScope(entenum.DataPermAll).Exec(l.ctx)
		if err != nil {
			return nil, errorx.NewInternalError(err.Error())
		}
...
func (l *InitDatabaseLogic) insertRoleMenuAuthorityData() error {
	...
	err = l.svcCtx.DB.Role.Update().AddMenuIDs(menuIds...).Where(role.IDEQ(1)).Exec(l.ctx)
...
func (l *InitDatabaseLogic) insertCasbinPoliciesData() error {
	...
	if adminData, roleErr := l.svcCtx.DB.Role.Query().Where(role.NameEQ("超级管理员")).First(l.ctx); roleErr == nil {
